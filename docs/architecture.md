...
| `apps/web-editor` | `@domorium/codemirror`      | Vite app deployed to `domorium.com` through GitHub Pages from `main` when web-related paths change; emits the editor at `/editor/` and the static pages around it — one route table in `src/site/` rendered to HTML at build time (ADR-0014); reads and writes a folder the reader grants through the browser's File System Access API, behind one gateway interface so the single-file and demo workspaces answer the same calls |
