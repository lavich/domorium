...
The `@gedcom` organization scope groups the packages under one stable project namespace, while the package suffixes directly describe their responsibilities. Package descriptions and keywords provide npm search discoverability.
...
The npm user `lavich`, as an owner of the `gedcom` organization, publishes the two scoped packages publicly with `--access public`.
...
- Publishing `@gedcom/language-server`; it remains an internal transport adapter.
