...
Publish Domorium's editor-independent GEDCOM libraries as public npm packages so that the standalone Obsidian repository can consume versioned releases instead of maintaining copied source code.

## Packages

The main `domorium` repository remains the source of truth for two independently versioned packages:

- `@domorium/validator`
- `@domorium/language-service`

The package names stay concise because the `@domorium` scope already identifies the GEDCOM-focused ecosystem. Package descriptions and keywords provide npm search discoverability.

Each package has its own version and changelog. `@domorium/language-service` declares a compatible released version range of `@domorium/validator`. npm workspaces continue to connect the local packages during monorepo development.
...
The npm user `lavich` creates the free public organization `domorium`, which owns the `@domorium` scope. The first public packages are published with `--access public`.
...
The standalone `lavich/domorium-obsidian` repository removes its copied `packages/validator` and `packages/language-service` workspaces. Its application declares `@domorium/language-service` as a normal npm dependency. `@domorium/validator` is declared directly only if the Obsidian source imports its public API; otherwise it remains a transitive dependency.
...
While a package is below `1.0.0`, consumers use a minor-pinned range such as `^0.1.0`; breaking changes require increasing the minor version. Dependency ranges in `@domorium/language-service` are updated whenever its minimum compatible validator version changes.
...
- Publishing `@domorium/lsp`; it remains an internal transport adapter.
