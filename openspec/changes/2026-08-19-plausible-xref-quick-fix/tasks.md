...
- [ ] 1.1 In `packages/validator/src/validator/rule-node.ts`, `case "pointer"`:
      ...
      a subject any more.
- [ ] 1.2 Update the three pointer assertions in `rule-node.test.ts` (around lines
      ...
      Add tests for: a pointer whose schema names no target tag, and a document
      declaring no record of the required type at all reporting the same sentence.
- [ ] 1.3 Confirm `formatValueSet` and `MAX_LISTED_VALUES` are now reachable only
      from `validateEnumeration`, and that nothing else in the package calls them.
- [ ] 1.4 `npm run build:libs`, so `language-service` compiles against the new `dist`
      rather than stale output.

## 2. Choosing a plausible candidate

- [ ] 2.1 Add `packages/language-service/src/libs/codeActions/nearestXref.ts`:
...
- [ ] 3.1 In `packages/language-service/src/libs/symbols/recordLabel.ts`, add an
      ...
      resolver or not.
- [ ] 3.2 Extend `recordLabel.test.ts`: a family named from both spouses, from one,
...
- [ ] 4.1 Add `nodes: ASTNode[]` to `CodeActionContext` and pass
...
- [ ] 4.2 Rewrite `unresolvedXrefActions` in
...
- [ ] 4.3 Rewrite the two tests this inverts rather than deleting them: "returns
...
- [ ] 5.1 `npm run check:conformance -- --update` and read the diff in
      `scripts/conformance-corpus.json` and `scripts/vendor-corpus.json`. The message
      text changes for every unresolved pointer; the violation count must not move.
      A moved count means a code changed with the message — find it before accepting
      the diff. Record the before and after counts in the pull request. Needs the
      network; say so if it could not run.

## 6. Documentation

- [ ] 6.1 Changelog entry by hand in `packages/validator/CHANGELOG.md` and
      `packages/language-service/CHANGELOG.md`.
- [ ] 6.2 `packages/language-service/README.md`: it has no code-actions section today
...
- [ ] 6.3 State the publish order in the pull request description: per ADR-0003,
      `validator` before `language-service`. No version is bumped and no tag is
      created here.
- [ ] 6.4 Check `TODO.md` for an item this completes, and confirm
      `docs/architecture.md` needs nothing — no layer moves and no dependency
      direction changes.
- [ ] 6.5 Open the follow-up issue for creation writing the reciprocal pointer,
