...
- [x] 1.1 Write `scripts/vendor-corpus.json` with the fourteen entries design.md
...
- [x] 2.1 Teach `scripts/check-conformance.mjs` to read a second corpus, keeping
      ...
      separately.
- [x] 2.2 Refuse a location that does not carry a full upstream revision, so an
      unpinned entry cannot be added later. Test it.
- [x] 2.3 Add the `summary` expectation shape — total, count per code, and a digest
...
- [x] 2.4 Extract the parts worth testing — normalising diagnostics, the digest,
...
- [x] 3.1 Run `npm run check:conformance -- --update`, read the recorded numbers
...
- [x] 3.2 Give each high-volume entry a one-line note saying what dominates it, so
...
- [x] 5.1 Extend the header comment of `scripts/check-conformance.mjs`: the second
      corpus, why nothing is vendored however permissive the licence, and what the
      two expectation shapes are for.
- [x] 5.2 Write `docs/adr/0011-fetch-corpora-rather-than-vendoring-them.md` from
      ...
      changelog entry.
- [x] 5.3 Make the `conformance` job in `.github/workflows/ci.yml` run against both
      corpora and fire when either corpus file or the script changes.

## 6. Checks

- [x] 6.1 Run `npm run check:conformance` from a clean checkout and confirm it
      passes, reports both corpora, and takes an acceptable time.
- [x] 6.2 Confirm `npm run check` neither fetches anything nor fails without a
      network.
- [x] 6.3 Run `npm run check` and record whether `check:jetbrains` ran or was
